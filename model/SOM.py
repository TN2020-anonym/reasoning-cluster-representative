    ...
    def __init_graph(self, alpha, sigma, dim): 
        ...
        with self._graph.as_default():
            ...
            self._location_vects = tf.constant(np.array(
                list(self._neuron_locations(self._m, self._n))))
            ...
            bmu_index = tf.argmin(tf.sqrt(tf.reduce_sum(
                tf.pow(tf.subtract(self._weightage_vects, tf.stack(
                    [self._vect_input for i in range(self._m*self._n)])), 2), 1)),
                                  0)
            ...
            bmu_distance_squares = tf.reduce_sum(tf.pow(tf.subtract(
                self._location_vects, tf.stack(
                    [bmu_loc for i in range(self._m*self._n)])), 2), 1)
            ...
            weightage_delta = tf.multiply(
                ...
                tf.subtract(tf.stack([self._vect_input for i in range(self._m*self._n)]),
